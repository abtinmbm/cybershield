...
def get_public_resource_user():
    ...
    for user_profile in UserProfile.objects.all():
        ...
        if (
            ...
            and hasattr(user_profile.profile_pic, "url")
            and os.path.exists(
                os.path.join(settings.MEDIA_ROOT, user_profile.profile_pic.name)
            )
        ):
...
def get_public_resource_moderator():
    ...
    for moderator in Moderator.objects.filter(status="active").all():
        ...
        username = (
            ...
            if hasattr(moderator.username, "username")
            else str(moderator.username)
        )

        # Check if profile_pic exists and has an associated file
        if (
            moderator.profile_pic
            and hasattr(moderator.profile_pic, "url")
            and os.path.exists(
                os.path.join(settings.MEDIA_ROOT, moderator.profile_pic.name)
            )
        ):
